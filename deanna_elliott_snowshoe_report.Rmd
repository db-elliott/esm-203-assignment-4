...
The mean weight of male snowshoe hares at Bonanza LTER is 90.5 g greater than the mean female weight; this is a 10.05% difference. If these samples were taken from populations with the same mean, there is a `r round(t_test$p.value*100, 2)`% chance of finding sample means *at least* as different as these, which is very remote. Additionally, the means of male and female weights do differ significantly (t-test | t(`r round(t_test$parameter, 0)`) = `r (t_test$statistic)`, p = `r round(t_test$p.value, 4)`). However, the effect size of the difference is small (Cohen's d |  d = `r round(cd$estimate, 2)`).
...
**Figure 3. Juvenile snowshoe hare hind foot length vs weight.** Measurements of juvenile hares in the Bonanza Creek LTER show a moderately positive correlation between hind foot length (in mm) and weight (in g). Generally, as the length of the hind foot increases, so does the animal's weight.
...
pr<- cor.test(juv_hares$hindft, juv_hares$weight)
